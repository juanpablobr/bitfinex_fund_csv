...
var hbconfig=config.apikey.huobipro;
var timeout=5*1000;

function get_auth() {
    var sign = hbconfig.trade_password + 'hello, moto';
...
function sign_sha(method, baseurl, path, data) {
    ...
    // console.log(meta);
    var hash = HmacSHA256(meta, hbconfig.secret);
...
function call_api(method, path, payload, body) {
    return new Promise(resolve => {
        var account_id = hbconfig.account_id_pro;
...
var HUOBI_PRO = {
    ...
    buy_limit: function(symbol, amount, price) {
        ...
        body["account-id"] = hbconfig.account_id_pro;
        body.type = "buy-limit";
        body.amount = amount;
        body.symbol = symbol;
        body.price = price;
